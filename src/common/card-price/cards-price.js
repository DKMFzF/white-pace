function createCardPrice(card) {
  ...
  const cardElement = cardTemplate
    .querySelector('.section-content__list-item-card-price')
    .cloneNode(true);

  cardElement.querySelector('.card-price__title').textContent =
    card.header.title;
  cardElement.querySelector('.card-price__price').textContent =
    card.header.price;
  cardElement.querySelector('.card-price__description-card').textContent =
...
function createCardListItemPrice(card, item, index, dataListCard) {
  ...
  // add specific-class
  if (index === 3 || index === 5)
    listItemCard.classList.add('card-price__list_additional-inset');
  if (
    (cardname == 'free' && dataListCard.length >= 2) ||
    (cardname == 'personal' && dataListCard.length >= 4)
  ) {
